...
interface OAuthButtonsProps {
  /** 'login' shows only Google. 'register' shows Google + GitHub + Apple */
  mode: 'login' | 'register';
}
...
export function OAuthButtons({ mode }: OAuthButtonsProps) {
  ...
  const providerConfig = [
    {
      ...
      icon: <GoogleIcon />,
      className:
        'border-border hover:border-[#4285F4]/60 hover:bg-[#4285F4]/5',
    },
    {
  ...
  return (
    <div className="space-y-2">
      {providerConfig.map(({ id, label, icon, className }) => (
        <button
          key={id}
          type="button"
          ...
          className={`
            relative w-full flex items-center justify-center gap-3 px-4 py-2
            rounded-xl border bg-background/50 text-sm font-semibold
